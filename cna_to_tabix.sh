...
) | bgzip > $1.sorted.gz;    # block gzip with a tool from htslib

tabix -s 20 -b 21 -e 22 -f -S 1 $1.sorted.gz;    # create index with tabix from htslib: -S 1 = skip first line (header)
